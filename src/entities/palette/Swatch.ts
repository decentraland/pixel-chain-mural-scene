...
export class Swatch extends Entity {
  ...
  constructor(
    private readonly index: SwatchIndex,
    onSwatchChangeListener: (color: ColorHex) => void
  ) {
    super()
    ...
    this.addComponent(
      new Transform({
        position: new Vector3(x, y, -0.001),
        scale: new Vector3(0.16, 0.16, 0.1)
      })
    )
    this.addComponent(
      new OnPointerDown(

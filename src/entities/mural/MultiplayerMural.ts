...
export class MultiplayerMural extends MultiplayerEntity<
  TilePosition,
  SwatchIndex,
  SwatchIndex[][]
> {
  ...
  protected runInitialLoad(): void {
    this.entityMural = Mural.initializeEmpty(
      MURAL_SIZE,
      MURAL_SIZE,
      (position) => {
        const tile = new Tile(position, (index) =>
          this.changeEntity(position, index)
        )
        tile.setParent(this)
        return tile
      }
  ...
  protected initializeWithFullState(fullState: SwatchIndex[][]): void {
    ...
    this.colorMural.getAllTiles().forEach(({ pos, value: idx }) => {
